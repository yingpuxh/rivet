// -*- C++ -*-
#ifndef RIVET_FastHiggsJets_HH
#define RIVET_FastHiggsJets_HH
...
namespace Rivet {
  ...
  class FastHiggsJets : public JetAlg {
  public:
    ...
    FastHiggsJets(const FinalState& fsp,
    ...
    FastHiggsJets(const FinalState& fsp,
             ...
             JetAlg::MuonsStrategy usemuons=JetAlg::ALL_MUONS,
             JetAlg::InvisiblesStrategy useinvis=JetAlg::NO_INVISIBLES)
      : FastHiggsJets(fsp, jdef, usemuons, useinvis, adef)
    {    }
    ...
    FastHiggsJets(const FinalState& fsp,
             ...
             JetAlg::InvisiblesStrategy useinvis=JetAlg::NO_INVISIBLES,
             fastjet::AreaDefinition* adef=nullptr)
      : FastHiggsJets(fsp, fastjet::JetDefinition(type, rparameter, recom), usemuons, useinvis, adef)
    {    }
    ...
    FastHiggsJets(const FinalState& fsp,
             ...
             JetAlg::MuonsStrategy usemuons=JetAlg::ALL_MUONS,
             JetAlg::InvisiblesStrategy useinvis=JetAlg::NO_INVISIBLES)
      : FastHiggsJets(fsp, type, recom, rparameter, usemuons, useinvis, adef)
    {    }
    ...
    FastHiggsJets(const FinalState& fsp,
             ...
             JetAlg::InvisiblesStrategy useinvis=JetAlg::NO_INVISIBLES,
             fastjet::AreaDefinition* adef=nullptr)
      : FastHiggsJets(fsp, fastjet::JetDefinition(plugin), usemuons, useinvis, adef)
    {
    ...
    FastHiggsJets(const FinalState& fsp,
             ...
             JetAlg::MuonsStrategy usemuons=JetAlg::ALL_MUONS,
             JetAlg::InvisiblesStrategy useinvis=JetAlg::NO_INVISIBLES)
      : FastHiggsJets(fsp, plugin, usemuons, useinvis, adef)
    {    }
    ...
    // FastHiggsJets(JetAlgName alg, double rparameter, double seed_threshold=1.0) { _initJdef(alg, rparameter, seed_threshold); }
    // /// Same thing as above, but without an FS (for when we want to pass the particles directly to the calc method)
    // /// @todo Does this work properly, without internal HeavyQuarks etc.?
    // FastHiggsJets(fastjet::JetAlgorithm type, fastjet::RecombinationScheme recom, double rparameter) { _initJdef(type, recom, rparameter); }
    // /// Same thing as above, but without an FS (for when we want to pass the particles directly to the calc method)
    // /// @todo Does this work properly, without internal HeavyQuarks etc.?
    // FastHiggsJets(fastjet::JetDefinition::Plugin* plugin) : _jdef(plugin), _plugin(plugin) {
    //   // _plugin.reset(plugin);
    ...
    /// Clone on the heap.
    DEFAULT_RIVET_PROJ_CLONE(FastHiggsJets);
