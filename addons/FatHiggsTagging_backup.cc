...
namespace Rivet {
  ...
  public:
	...
	_h_toppt          = bookHisto1D("toppt",              26,  200.,  1500.);
      ...
      _h_htmissbefore     = bookHisto1D("htmissbefore",       36,   0.0,   2800.);
      _h_htmiss		  = bookHisto1D("htmiss",             36,   0.0,   2800.);
      ...
      _h_htbefore         = bookHisto1D("htbefore",           36, 0.0,  5000.);
      _h_ht               = bookHisto1D("ht",    	      36, 0.0,  5000.);
      ...
      _h_regularjetssize  = bookHisto1D("regularjetssize",     11,   2.,   13.);
      _h_bjetssize        = bookHisto1D("bjetssize",           6,   0.,   6.);
    ...
    //  const vector<DressedLepton> dressedphotons = apply<DressedLeptons>(event, "dressedphotons").dressedLeptons();     
      const Jets& trackjets  = apply<FastJets>(event, "trackjets").jetsByPt(Cuts::pT > 10*GeV && Cuts::abseta < 4.0);

      const Jets& calojets  = apply<FastJets>(event, "calojets").jetsByPt(Cuts::pT > 10*GeV && Cuts::abseta < 4.0);//&& Cuts::abseta < 3.0);
      ...
      _h_regularjetssizebefore->fill(js.size(),                                               event.weight());
      ...
      for (unsigned ifjet = 0 ; ifjet < js.size(); ifjet++) {
       ...
       int btagss(0.);
       for (unsigned i=0; i<trackflavors.size(); i++) if ( trackflavors[i] == "bjet" ) btagss++;
       if ( btagss>0 ) btag=btag+1; 
       trackflavors.clear();
      }
    ...
    for (unsigned ifjet = 0 ; ifjet < js.size(); ifjet++) {
     // loop over track jets to determin fat jet flavor and muon corrections 
      ...
      int btagss(0.);
      for (unsigned i=0; i<trackflavor.size(); i++) if ( trackflavor[i] == "bjet" ) btagss++;
      if ( btagss>0 ) btags=btags+1;
