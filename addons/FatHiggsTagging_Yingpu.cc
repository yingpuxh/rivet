...
namespace Rivet {
  ...
  public:
    ...
    void init() {
     ...
     // regular jets
     FastJets trackregularjets(fs, FastJets::ANTIKT, 0.4, JetAlg::NO_MUONS, JetAlg::NO_INVISIBLES);
     addProjection(trackregularjets, "trackregularjets");
	...
	_h_toppt          = bookHisto1D("toppt",              26,  200.,  1500.);
      ...
      _h_fatjetpt         = bookHisto1D("fatjetpt",           26,  0.,  1500.);
    ...
    //  _h_fatjetnsub21     = bookHisto1D("fatjetnsub21",       16,   0.0,    1.6);
     ...
     // _h_fatjetd2         = bookHisto1D("fatjetd2",           25,   0.0,    5.0);
      ...
      _h_regularjetseta4  =bookHisto1D("regularjetseta4",    25,   -2.5,    2.5);
      _h_regularjetseta5  =bookHisto1D("regularjetseta5",    25,   -2.5,    2.5);
      _h_regularjetseta6  =bookHisto1D("regularjetseta6",    25,   -2.5,    2.5);
      _h_regularjetseta7  =bookHisto1D("regularjetseta7",    25,   -2.5,    2.5);
      _h_regularjetseta8  =bookHisto1D("regularjetseta8",    25,   -2.5,    2.5);
      _h_regularjetseta9  =bookHisto1D("regularjetseta9",    25,   -20.0,    20.0);
      _h_regularjetseta10  =bookHisto1D("regularjetseta10",  25,   -2.5,    2.5);
      ...
      _h_regularjetspt4  =bookHisto1D("regularjetspt4",    26,   0.,    1500.);
      _h_regularjetspt5  =bookHisto1D("regularjetspt5",    26,   0.,    1500.);
      _h_regularjetspt6  =bookHisto1D("regularjetspt6",    26,   0.,    1500.);
      _h_regularjetspt7  =bookHisto1D("regularjetspt7",    26,   0.,    1500.);
      _h_regularjetspt8  =bookHisto1D("regularjetspt8",    26,   0.,    1500.);
      _h_regularjetspt9  =bookHisto1D("regularjetspt9",    26,   0.,    1500.);
      _h_regularjetspt10  =bookHisto1D("regularjetspt10",  26,   0.,    1500.);
      ...
      _h_regularjet4dphimet    = bookHisto1D("regularjet4dphimet",      20,   0.0,    1.0);
      _h_regularjet5dphimet    = bookHisto1D("regularjet5dphimet",      20,   0.0,    1.0);
      _h_regularjet6dphimet    = bookHisto1D("regularjet6dphimet",      20,   0.0,    1.0);
      _h_regularjet7dphimet    = bookHisto1D("regularjet7dphimet",      20,   0.0,    1.0);
      _h_regularjet8dphimet    = bookHisto1D("regularjet8dphimet",      20,   0.0,    1.0);
      _h_regularjet9dphimet    = bookHisto1D("regularjet9dphimet",      20,   0.0,    1.0);
      _h_regularjet10dphimet   = bookHisto1D("regularjet10dphimet",      20,   0.0,    1.0);


     _h_regularjetsht          = bookHisto1D("regularjetsht",           26,    0.,   1500.);
     _h_alljetsht          = bookHisto1D("alljetsht",           26,    0.,   1500.);          
    ...
    void analyze(const Event& event) {
      ...
      const Jets& trackregularjets  = apply<FastJets>(event, "trackregularjets").jetsByPt(Cuts::pT > 10*GeV && Cuts::abseta < 2.5); 
	...
	vetoEvent;
     ...
     //veto if there is no regular jets in the event
      if (trackregularjets.size()==0) vetoEvent;     

    vector<unsigned> trackjetfjidx1; trackjetfjidx1.clear();
      for(unsigned itjet = 0 ; itjet < trackjets.size(); itjet++) {
        double drmin(999.);
        unsigned ifjetclose(0);
        for(unsigned ifjet = 0 ; ifjet < trackregularjets.size(); ifjet++) {
          double tmp=deltaR(trackjets[itjet], trackregularjets[ifjet]);
          if(tmp < drmin){
            drmin=tmp;
            ifjetclose=ifjet;
          }
    ...
    if(drmin<1.1 && trackjets[itjet].constituents().size()>1 ) {
        ...
        std::cout<<" regular jets in the event " << trackregularjets.size() << " track jets " << trackjets.size()<< std::endl;
	...
	if(drmin<1.1 && trackjets[itjet].constituents().size()>1 ) { 
     ...
     // double beta = 1.0;
      // tau21wta 
      vector<PseudoJet> trimmedregularjets; trimmedregularjets.clear();
      for (unsigned ifjet = 0 ; ifjet < trackregularjets.size(); ifjet++) {
        double rsub = 0.2;
        double fcut = 0.05;
        fastjet::Filter treetrimmer(rsub, fastjet::SelectorPtFractionMin(fcut));
        PseudoJet pjets = treetrimmer(trackregularjets[ifjet]);
        if ( pjets.perp()<10 )     continue;
        if ( abs(pjets.eta())>2.5) continue;
        ...
        if(itjet<trackjets.size()-1)
           ...
           trimmedregularjets.push_back(pjets); 
                }
...
//	if(std::isnan(nSub21(pjet))){
//	  std::cout<<"WARNING::NAN in jet substructure!--nSub21"<<std::endl;
//	  continue;
//	  	}
        
// if(std::isnan(c2(pjet))){
  //        std::cout<<"WARNING::NAN in jet substructure!--c2"<<std::endl;
    //      continue;
      //          }


// if(std::isnan(d2(pjet))){
  //        std::cout<<"WARNING::NAN in jet substructure!--d2"<<std::endl;
    //      continue;
      //          }
...
if (trimmedregularjets.size()!=trackregularjets.size()) vetoEvent;
 if(trimmedfatjets.size()!=1) vetoEvent;
// fill in a histogram here (Yingpu)
...
// fat jet related quantities
     ...
     // _h_fatjetc2->fill(c2(trimmedfatjets[0]),                                            event.weight());
      ...
      _h_fatjetdphimet->fill(deltaPhi(trimmedfatjets[0].phi_02pi(),met.phi(ZERO_2PI))/PI, event.weight());
            ...
            for(unsigned itjet = 0 ; itjet < trackregularjets.size(); itjet++) {
_h_regularjetsht->fill(trimmedregularjets[itjet].perp()/GeV,                                 event.weight()); 
_h_alljetsht->fill(trimmedregularjets[itjet].perp()/GeV,                                 event.weight());
}
_h_alljetsht->fill(trimmedfatjets[0].perp()/GeV,                                     event.weight());

            for(unsigned itjet = 0 ; itjet < trackregularjets.size(); itjet++) {
if (itjet==0)    
{ _h_regularjetseta0->fill(trimmedregularjets[itjet].eta(),                                  event.weight());
_h_regularjetspt0->fill(trimmedregularjets[itjet].perp()/GeV,                                 event.weight());
_h_regularjet0dphimet->fill(deltaPhi(trimmedregularjets[itjet].phi_02pi(),met.phi(ZERO_2PI))/PI, event.weight());
}
if (itjet==1)
{ _h_regularjetseta1->fill(trimmedregularjets[itjet].eta(),                                  event.weight());
_h_regularjetspt1->fill(trimmedregularjets[itjet].perp()/GeV,                                 event.weight());
_h_regularjet1dphimet->fill(deltaPhi(trimmedregularjets[itjet].phi_02pi(),met.phi(ZERO_2PI))/PI, event.weight());
}
if (itjet==2)
{ _h_regularjetseta2->fill(trimmedregularjets[itjet].eta(),                                  event.weight());
_h_regularjetspt2->fill(trimmedregularjets[itjet].perp()/GeV,                                 event.weight());
_h_regularjet2dphimet->fill(deltaPhi(trimmedregularjets[itjet].phi_02pi(),met.phi(ZERO_2PI))/PI, event.weight());
}
if (itjet==3)
{ _h_regularjetseta3->fill(trimmedregularjets[itjet].eta(),                                  event.weight());
_h_regularjetspt3->fill(trimmedregularjets[itjet].perp()/GeV,                                 event.weight());
_h_regularjet3dphimet->fill(deltaPhi(trimmedregularjets[itjet].phi_02pi(),met.phi(ZERO_2PI))/PI, event.weight());
}
if(itjet==4)
{   _h_regularjetseta4->fill(trimmedregularjets[itjet].eta(),                                  event.weight());
_h_regularjetspt4->fill(trimmedregularjets[itjet].perp()/GeV,                                 event.weight());
_h_regularjet4dphimet->fill(deltaPhi(trimmedregularjets[itjet].phi_02pi(),met.phi(ZERO_2PI))/PI, event.weight());
}
if(itjet==5)
{ _h_regularjetseta5->fill(trimmedregularjets[itjet].eta(),                                  event.weight());
_h_regularjetspt5->fill(trimmedregularjets[itjet].perp()/GeV,                                 event.weight());
_h_regularjet5dphimet->fill(deltaPhi(trimmedregularjets[itjet].phi_02pi(),met.phi(ZERO_2PI))/PI, event.weight());
}
if (itjet==6)
{ _h_regularjetseta6->fill(trimmedregularjets[itjet].eta(),                                  event.weight());
_h_regularjetspt6->fill(trimmedregularjets[itjet].perp()/GeV,                                 event.weight());
_h_regularjet6dphimet->fill(deltaPhi(trimmedregularjets[itjet].phi_02pi(),met.phi(ZERO_2PI))/PI, event.weight());
}
if(itjet==7)
{ _h_regularjetseta7->fill(trimmedregularjets[itjet].eta(),                                  event.weight());
_h_regularjetspt7->fill(trimmedregularjets[itjet].perp()/GeV,                                 event.weight());
_h_regularjet7dphimet->fill(deltaPhi(trimmedregularjets[itjet].phi_02pi(),met.phi(ZERO_2PI))/PI, event.weight());
}
if(itjet==8)
{ _h_regularjetseta8->fill(trimmedregularjets[itjet].eta(),                                  event.weight());
_h_regularjetspt8->fill(trimmedregularjets[itjet].perp()/GeV,                                 event.weight());
_h_regularjet8dphimet->fill(deltaPhi(trimmedregularjets[itjet].phi_02pi(),met.phi(ZERO_2PI))/PI, event.weight());
}
if (itjet==9)
{ _h_regularjetseta9->fill(trimmedregularjets[itjet].eta(),                                  event.weight());
_h_regularjetspt9->fill(trimmedregularjets[itjet].perp()/GeV,                                 event.weight());
_h_regularjet9dphimet->fill(deltaPhi(trimmedregularjets[itjet].phi_02pi(),met.phi(ZERO_2PI))/PI, event.weight());
}
if(itjet==10)
{ _h_regularjetseta10->fill(trimmedregularjets[itjet].eta(),                                  event.weight());
_h_regularjetspt10->fill(trimmedregularjets[itjet].perp()/GeV,                                 event.weight());
_h_regularjet10dphimet->fill(deltaPhi(trimmedregularjets[itjet].phi_02pi(),met.phi(ZERO_2PI))/PI, event.weight());
}
if (itjet>10)
break;
}
...
//      _h_regularjetseta0->fill(trimmedregularjets[0].eta(),                                  event.weight()); 
  //    _h_regularjetseta1->fill(trimmedregularjets[1].eta(),                                  event.weight());
    //  _h_regularjetseta2->fill(trimmedregularjets[2].eta(),                                  event.weight());
     // _h_regularjetseta3->fill(trimmedregularjets[3].eta(),                                  event.weight());
     // _h_regularjetseta4->fill(trimmedregularjets[4].eta(),                                  event.weight());
     // _h_regularjetseta5->fill(trimmedregularjets[5].eta(),                                  event.weight());
     // _h_regularjetseta6->fill(trimmedregularjets[6].eta(),                                  event.weight());
     // _h_regularjetseta7->fill(trimmedregularjets[7].eta(),                                  event.weight());
     // _h_regularjetseta8->fill(trimmedregularjets[8].eta(),                                  event.weight());
     // _h_regularjetseta9->fill(trimmedregularjets[9].eta(),                                  event.weight());
     // _h_regularjetseta10->fill(trimmedregularjets[10].eta(),                                event.weight());
...
// event properties
	...
	//std::cout<<"end of analyse"<<std::endl;
...
//	if (partontops.size() >= 2) if ( partontops[1].perp()>250 * GeV ) _h_toppt->fill(partontops[1].perp()/GeV,  event.weight());
  //    }
    ...
    //  _h_fatjetpt->fill(trimmedfatjets[0].perp()/GeV,                                     event.weight());
     ...
     // _h_fatjetntjets->fill(tfjntracks[0],                                                event.weight());
     // _h_fatjetdphimet->fill(deltaPhi(trimmedfatjets[0].phi_02pi(),met.phi(ZERO_2PI))/PI, event.weight());
     // _h_addon1->fill(trimmedfatjets[0].perp()/GeV,                                        event.weight());      
...
//
   ...
   // }//analyze
    ...
    void finalize() {
     ...
     _h_regularjetseta2->scaleW(norm);
      ...
      _h_regularjetseta4->scaleW(norm);
      _h_regularjetseta5->scaleW(norm);
      _h_regularjetseta6->scaleW(norm);
      _h_regularjetseta7->scaleW(norm);
      _h_regularjetseta8->scaleW(norm);
      _h_regularjetseta9->scaleW(norm);
      _h_regularjetseta10->scaleW(norm);       

      _h_regularjetspt0->scaleW(norm);
      _h_regularjetspt1->scaleW(norm);
     _h_regularjetspt2->scaleW(norm);
      _h_regularjetspt3->scaleW(norm);
      _h_regularjetspt4->scaleW(norm);
      _h_regularjetspt5->scaleW(norm);
      _h_regularjetspt6->scaleW(norm);
      _h_regularjetspt7->scaleW(norm);
      _h_regularjetspt8->scaleW(norm);
      _h_regularjetspt9->scaleW(norm);
      _h_regularjetspt10->scaleW(norm);
      ...
      _h_regularjet4dphimet->scaleW(norm);
      _h_regularjet5dphimet->scaleW(norm);
      _h_regularjet6dphimet->scaleW(norm);
      _h_regularjet7dphimet->scaleW(norm);
      _h_regularjet8dphimet->scaleW(norm);
      _h_regularjet9dphimet->scaleW(norm);
      _h_regularjet10dphimet->scaleW(norm);
      
       _h_regularjetsht->scaleW(norm);
       _h_alljetsht->scaleW(norm);
 ...
 //   Histo1DPtr   _h_fatjetc2;
   // Histo1DPtr   _h_fatjetd2;
    ...
    Histo1DPtr   _h_regularjetseta4;
    Histo1DPtr   _h_regularjetseta5;
    Histo1DPtr   _h_regularjetseta6;
    Histo1DPtr   _h_regularjetseta7;
    Histo1DPtr   _h_regularjetseta8;
    Histo1DPtr   _h_regularjetseta9;
    Histo1DPtr   _h_regularjetseta10;
    Histo1DPtr   _h_regularjetspt0;
    Histo1DPtr   _h_regularjetspt1;
    Histo1DPtr   _h_regularjetspt2;
    Histo1DPtr   _h_regularjetspt3;
    Histo1DPtr   _h_regularjetspt4;
    Histo1DPtr   _h_regularjetspt5;
    Histo1DPtr   _h_regularjetspt6;
    Histo1DPtr   _h_regularjetspt7;
    Histo1DPtr   _h_regularjetspt8;
    Histo1DPtr   _h_regularjetspt9;
    Histo1DPtr   _h_regularjetspt10;
    ...
    Histo1DPtr   _h_regularjet4dphimet;
    Histo1DPtr   _h_regularjet5dphimet;
    Histo1DPtr   _h_regularjet6dphimet;
    Histo1DPtr   _h_regularjet7dphimet;
    Histo1DPtr   _h_regularjet8dphimet;
    Histo1DPtr   _h_regularjet9dphimet;
    Histo1DPtr   _h_regularjet10dphimet;
    Histo1DPtr   _h_regularjetsht;
    Histo1DPtr   _h_alljetsht;    
